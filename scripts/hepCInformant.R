...
hepC <- hepC %>% rename("Status" = "Category")
...
hepCInformant <- hepCInformant %>%
  info_snippet(
    "status_levels",
    fn = snip_list("Status", limit = 10, sorting = "inseq")
  ) %>%
  info_columns(
    columns = "Status",
    info = "A discrete (nominal) variable indicating disease state and blood donor status. Possible values: {status_levels}"
  ) %>%
  incorporate()
